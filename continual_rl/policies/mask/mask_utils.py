...
class MultitaskMaskLinear(nn.Linear):
    ...
    # NOTE quick fix version
    def forward(self, x, task_id):
        #self.task = task_id
        assert self.task == task_id, 'an error ocurred. self.task ({0}) != task_id ({1})'.format(self.task, task_id)
    ...
    def consolidate_mask(self):
        ...
        if self.task < self.num_tasks_learned:
            ...
            # task_train_end(...) method in mask_policy.py class.
    ...
    def set_task(self, task, new_task=False):
        ...
        if self.new_mask_type == NEW_MASK_LINEAR_COMB and new_task:
            if task > 0:
                k = task + 1
                # set the coeff for the new task to a fixed starting value of 0.5.
                # this is set to 0.5 for now. The remaining prob are shared equally
