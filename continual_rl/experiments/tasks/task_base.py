...
class TaskBase(ABC):
    ...
    def _log_policy_net(self, policy, summary_writer, total_log_timesteps):
        for tag, value in policy.impala_trainer.learner_model.named_parameters():
            tag = tag.replace('.', '/')
            summary_writer.add_histogram(tag, value.detach().cpu().numpy(), total_log_timesteps, bins=1000)
    ...
    def _run(self, task_spec, run_id, policy, summary_writer, output_dir, timestep_log_offset, wait_to_report,
        ...
        while task_timesteps < task_spec.num_timesteps:
            ...
            if len(returns_to_report) > 0 and not wait_to_report:
                ...
                # log network parameters for debugging purpose.
                self._log_policy_net(policy, summary_writer, total_log_timesteps)
