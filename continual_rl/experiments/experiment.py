...
class Experiment(object):
    ...
    def _run_continual_eval(self, task_run_id, policy, summary_writer, total_timesteps):
        ...
        for test_task_run_id, test_task in enumerate(self.tasks):
            ...
            # NOTE mask method specific
            try:
                int_task_id = int(test_task.task_id.split('_')[-1])
            except:
                int_task_id = int(test_task.task_id.split('_')[-2])
            policy.task_eval_start(int_task_id)
            #policy.task_eval_start(test_task_run_id)
    ...
    def _run(self, policy, summary_writer):
        ...
        for cycle_id in range(start_cycle_id, self._cycle_count):
            for task_run_id, task in enumerate(self.tasks[start_task_id:], start=start_task_id):
                ...
                # NOTE mask method specific
                try:
                    int_task_id = int(task.task_id.split('_')[-1])
                except:
                    int_task_id = int(task.task_id.split('_')[-2])
                policy.task_train_start(int_task_id)
                #policy.task_train_start(task_run_id)
